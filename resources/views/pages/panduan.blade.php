...
<div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 justify-items-center">
    ...
    @foreach($steps as $index => $step)
        <div class="bg-white border-2 border-black rounded-lg shadow-black shadow-[6px_6px_0_0] overflow-hidden flex flex-col w-[180px]">
            
            {{-- Gambar + Angka --}}
            <div class="relative w-full">
                {{-- Angka lingkaran --}}
                <div class="absolute top-2 left-2 w-8 h-8 rounded-full bg-white border-2 border-black flex items-center justify-center font-bold text-xs z-10">
                    {{ $index + 1 }}
                </div>

                {{-- Gambar --}}
                <img src="{{ $images[$index] }}" alt="{{ $step }}" class="w-full h-[220px] object-cover border-b-2 border-black">
            </div>

            {{-- Judul --}}
            <div class="p-2 flex-1 flex items-center justify-center">
                <p class="text-sm font-semibold text-center">{{ $step }}</p>
            </div>
        </div>
    @endforeach
</div>
